...
export const BRAINSTORM: Record<string, { underutilized: CollectIdea[]; buildNext?: CollectIdea[]; opportunities: CollectIdea[] }> = {
  ...
  attribution: {
    ...
    buildNext: [
      { label: "Keyword + match type", why: "Match type AND the {keyword} param are both captured now — but data only flows once a Google Ads tracking template actually carries keyword={keyword}; nothing arrives from Meta traffic, so this waits on a Google campaign." },
      { label: "Device type at click", why: "The {device} param is captured when present — add device={device} to a Google Ads tracking template to populate the device at the AD CLICK, catching sellers who click on a phone in the driveway but finish on a desktop." },
      { label: "Physical location ID", why: "The {loc_physical_ms} param is captured when present — add it to a Google Ads tracking template and you'll see the geographic area of each click before the inspection logistics ever come up." },
  ...
  referrals: {
    underutilized: [
      ...
      { label: "Referral code redemption status", why: "A lead typing a referral code now auto-matches back to the referral record and flips it from 'new' to 'qualified' — plus the code shows on the lead card and boosts its score, so redemptions track themselves." },
    ],
    buildNext: [
  ...
  chat: {
    underutilized: [
      ...
      { label: "Current page context", why: "The start page is stored AND the latest page now updates on every message — the chat panel shows 'On: /path', so mid-conversation you can see the visitor just opened the booking page and stalled." },
    ],
    buildNext: [
  ...
  geo: {
    underutilized: [
      ...
      { label: "Timezone vs. form time", why: "An IP timezone outside Canada now flags as an amber 'Timezone check' warning on the profile — a 'local' Alberta lead browsing from another time zone is a travel/VPN/out-of-province tell." },
      { label: "ASN-based repeat detection", why: "Leads are counted per network (ASN) across the whole lead table — 2+ other leads from the same network shows as a 'Shared network' warning on the profile, exposing a single actor spamming the form." },
      { label: "Foreign-number flag", why: "The IP's international calling code is compared against +1 — a mismatch shows as a 'Foreign network' warning on the profile, catching offshore fake sellers the country-only check misses." },
    ],
    opportunities: [
  ...
  metaAds: {
    underutilized: [
      ...
      { label: "Region + DMA breakdown", why: "Spend and leads split by locality in the 'By region' card (Calgary vs Edmonton vs rural), so you can see which towns produce cheap sellable cars and stop paying to reach areas outside a sane inspection-drive radius." },
      { label: "Placement-level cost-per-lead", why: "The 'By placement' card shows whether Reels, Stories, or FB Feed produce the cheapest leads, so budget stops leaking into placements that generate junk 'sell my car' clicks." },
      { label: "Ad relevance diagnostics", why: "Meta's quality / engagement / conversion rankings now ride the creative table as pills — a losing ad tells you whether it's dying on the creative, the hook, or the offer, instead of guessing." },
    ],
    buildNext: [
  ...
  ga4Data: {
    underutilized: [
      ...
      { label: "Hour and day-of-week", why: "Both sides are live now — the first-party lead-arrival heatmap plus GA4's 'When visitors browse' day×hour heatmap — so ad budget and drip sends can be timed to when sellers are actually looking, not just when they submit." },
    ],
    buildNext: [
      { label: "Source/medium by conversion", why: "Fetched and rendered ('Leads by source (GA4)') — the rows stay empty until generate_lead is marked as a Key event in GA4 Admin → Events; once toggled, GA4's own leads-per-source appears with zero further code." },
      ...
      { label: "Form-funnel key events", why: "Every stage now reaches GA4 — browser step events plus server-side contacted / offer_sent / booked / closed — the one remaining step is marking generate_lead as a Key event in the GA4 admin UI so conversion reports reflect the real funnel." },
  ...
  gtag: {
    ...
    buildNext: [
      ...
      { label: "Offer/booking as conversions", why: "Every stage transition now fires server-side (contacted, offer_sent with the offer value, booked, closed with real margin) — all that's left is marking them as Key events in the GA4 admin UI." },
  ...
  clarity: {
    underutilized: [
      ...
      { label: "Unmask non-sensitive fields", why: "The vehicle inputs (year / make / model / trim / mileage / VIN) are unmasked in recordings while name, phone, email and the damage note stay masked — so you can actually see what car details sellers typed and where they hesitated." },
    ],
    buildNext: [
  ...
  email: {
    underutilized: [
      ...
      { label: "Delivery-delayed events", why: "Resend's delivery_delayed (greylisting / soft-failure) events show as a 'Delivery delayed' row on the profile — an offer email silently stuck in retry no longer looks identical to a seller ghosting." },
      { label: "Tag emails by stage", why: "Every send is tagged in Resend with its kind (offer, day-10, winback…) plus the lead id, so open/click rates slice by exactly which email type converts — and any email traces straight back to its lead." },
      { label: "Idempotency keys", why: "Every template send now carries a deterministic idempotency key (offer emails keyed by the amount, so a NEW price still sends) — a cron retry or webhook double-fire can never double-send the same email." },
    ],
    buildNext: [
