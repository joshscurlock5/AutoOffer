...
export const BRAINSTORM: Record<string, { underutilized: CollectIdea[]; opportunities: CollectIdea[] }> = {
  leads: {
    underutilized: [
      ...
      { label: "Condition note free-text", why: "The seller's typed condition note (e.g. 'needs a windshield', 'salvage title', 'kids trashed the back seat') is captured but if it isn't parsed into keyword flags it can't pre-adjust the offer or warn the owner before he wastes a drive to inspect." },
    ...
    opportunities: [
      { label: "Field-level timing & hesitation", why: "Per-field timing shows where sellers pause or stall (usually price expectations or the phone field), letting you shorten the form or reword the exact question that's bleeding the most abandons — a proven, standard form-analytics feature." },
      { label: "Abandoned-form partial capture", why: "Saving each field as it's filled creates a recoverable partial lead (car + email, no phone) so an abandoned form becomes a follow-up email instead of a total loss — the highest-ROI addition for any lead form and the abandoned-cart of car-buying." },
      { label: "Input method detection", why: "Detecting whether the phone/email was typed, pasted, or browser-autofilled flags fake or bot leads (autofilled junk) versus a genuine seriously-selling human, saving the operator from chasing dead numbers." },
      { label: "IP geolocation to city", why: "The seller's IP resolves to a city/region so you know the pickup distance and whether the car is even inside your Alberta service radius before you invest a call — cheap to add and prevents wasted drives." },
      ...
      { label: "VIN or license plate", why: "An optional VIN/plate field pulls exact trim, options, and accident/lien history for a far more accurate pre-inspection offer, though many private sellers hesitate to share it so it's best kept optional." },
